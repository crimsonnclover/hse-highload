...
5. Портфель - открытые позиции, доступные средства и требуемое обеспечение (для срочного рынка).
...
Примечание: 13 заявок/день = 8 подач + 5 снятий/замен (см. §2.2.3). Журналы заявок и сделок хранятся централизованно. Профиль + позиции для всей базы 40,1 млн: $40{,}1 \times 10^6 \times 1{,}1\text{ КБ} \approx 44\text{ ГБ}$.
...
За основу берём среднедневное число сделок - 4,1 млн на всех рынках (2022) [[9]](https://sr2022.moex.com/ru/1/2/1/index.html). Число заявок на одну сделку оцениваем через статистику SEC. Публикуемое отношение trade-to-order по акциям ~3,5% считается по объёму бумаг, а не по числу ордеров [[10]](https://www.sec.gov/data-research/statistics-data-visualizations/trade-order-volume-ratios), поэтому напрямую инвертировать его некорректно. Для оценки берём cancel-to-trade ratio (число отмен на сделку), которое исторически составляет ~19-22 [[10]](https://www.sec.gov/data-research/statistics-data-visualizations/trade-order-volume-ratios); с учётом самой сделки это ~20-23 заявки.

Важная оговорка: данные SEC отражают американский рынок с высокой HFT-активностью. На MOEX доля HFT исторически ниже, поэтому реальный cancel-to-trade ratio может быть меньше (ближе к 5-10). Принимаем 25 как верхнюю оценку с запасом на снятия и замены:

$$
N_{orders}/\text{день} = 4{,}1 \times 10^6 \times 25 \approx 1{,}03 \times 10^8 \text{ заявок/день}
$$

Это подтверждается независимым источником: по данным MOEX, система обрабатывает до 250 млн заявок в день [[15]](https://www.tadviser.ru/index.php/Статья:Информационные_технологии_в_Московской_бирже). Инженеры биржи приводили реальные профили нагрузки с пиками до ~200 тыс. тх/с в коротком окне [[16]](https://habr.com/ru/companies/moex/articles/444300/).
...
| Задокументированная ёмкость (на рынок) | 38 000 тх/с | нижняя граница [[11]](https://www.dataspace.ru/company/press-center/it-dlya-investitsionnoy-ekosistemy/) |
| Задокументированная ёмкость (максимум) | 110 000 тх/с | верхняя граница [[11]](https://www.dataspace.ru/company/press-center/it-dlya-investitsionnoy-ekosistemy/) |
| Проектная цель (агрегат, с запасом 36%) | 150 000 тх/с | 110k + 36% |

Пояснение по цифрам: 110 000 тх/с - это задокументированная суммарная ёмкость MOEX (ASTS + SPECTRA). Проектная цель 150 000 тх/с - это наш design target с запасом на рост. В списке серверов (§11) ёмкость по шардам (70k ASTS + 40k SPECTRA = 110k) соответствует задокументированной базе; дополнительный запас закладывается через резервные шарды и горизонтальное масштабирование.
...
| Преторговый риск-чек (только SPECTRA) | торговый | ~200 | ~15 000 | пропорционально доле срочного рынка в потоке |
...
| Задокументированная ёмкость | 110 тыс. тх/с | [[11]](https://www.dataspace.ru/company/press-center/it-dlya-investitsionnoy-ekosistemy/) |
| Проектная цель (с запасом) | 150 тыс. тх/с | +36% к 110k |
...
erDiagram
    ...
    ACCOUNT ||--o| MARGIN_STATE : "имеет-ГО (только SPECTRA)"
...
| RISK_PARAM | Риск-ставки НКЦ | ~3 000 | вечер + интрадей | в памяти риск-чека | Strong |
...
| TRADE | Сделки | ~4,1 млн/день | ~4 000 (пик) | 8 000 | Strong |
| POSITION | Позиции | ~300 млн | ~4 000 (по сделкам) | 6 000 | Strong |
| MARGIN_STATE | ГО/вариационная маржа (только SPECTRA-счета) | ~10 млн | ~1 000 (по сделкам и клиринг. сессиям) | в памяти риск-чека | Strong |
| ORDER_LOG | Журнал заявок (audit) | append, ~8 ТБ/5 лет | 110 000 | аналитика | Strong |
| TRADE_LOG | Журнал сделок | append, ~0,3 ТБ/5 лет | ~4 000 | аналитика | Strong |
...
INSTRUMENT.tick_size - минимальный шаг цены, хранится как целое (например, для акции с шагом 0,01 руб. и ценой 150,00 руб. в ORDER.price хранится 15000). Матчинг сравнивает цены как целые числа без деления.

POSITION.qty - количество инструментов (для акций ASTS) или нетто-количество контрактов (для фьючерсов SPECTRA). Биржа не хранит среднюю цену позиции - это задача брокера. P&L для фьючерсов считается через вариационную маржу (MARGIN_STATE.vm), а не через avg_price.

MARGIN_STATE - только для счетов срочного рынка (SPECTRA). Фондовый рынок не использует портфельное маржирование: брокер заранее депонирует денежное обеспечение, и контроль остатка ведёт сам брокер. Write QPS в MARGIN_STATE невысокий: таблица обновляется при проведении клиринговых сессий (дважды в день) и при прохождении сделок через Kafka-потребителя - но не при каждой входящей заявке. Риск-чек читает актуальное состояние из in-memory копии, а не из PostgreSQL.

INVESTOR - биржа хранит расчётные коды и коды клиентов, присвоенные брокерами при регистрации для клиринговых целей. Личные данные (ФИО, паспорт) хранятся у брокеров и в НРД, а не на бирже.

SESSION - относится к публичному информационному контуру (веб-сайт, ISS-API) и архитектурно изолирована от торговых таблиц. Физически хранится в отдельном Redis-кластере.

## 5.3 Требования к согласованности

- Strong (полная согласованность). ORDER, TRADE, POSITION, MARGIN_STATE, ORDER_LOG, TRADE_LOG - ядро не может терять, дублировать или переставлять заявки; матчинг - единственный источник истины. Реализуется через однопоточное ядро на инструмент с детерминированной обработкой + журнал событий с синхронной репликацией.
...
flowchart TD
    ...
    subgraph Core ["Торговое ядро"]
        direction LR
        ME1["Матчинг ASTS<br/>Шарды по инструментам<br/>Sequencer per shard"]:::core
        ME2["Матчинг SPECTRA<br/>Шарды по инструментам<br/>Sequencer per shard"]:::core
        JRN["Журнал событий WAL<br/>per shard, 3 реплики"]:::core
    ...
    JRN -->|append-only поток| KAFKA[(Kafka<br/>Распределение событий)]:::mq
...
| :------ | :--------------- | :----------- | :------------- |
| Стакан (in-memory в ядре) | Собственное ядро (C++), отдельный объект от QUOTE_CACHE | По инструменту (горячие - отдельно) | Primary + горячий standby; восстановление replay журнала, синхронная репликация в резервный ЦОД |
| QUOTE_CACHE | Redis | По инструменту (хэш-слоты) | 3 master + 3 slave; восстановление из потока Kafka/MD-паблишера |
| ORDER_LOG / TRADE_LOG (WAL) | Журнал событий (append-only) + Kafka | По партиции рынка (per shard) | 3 реплики (кворум), реплика в резервном ЦОД |
...
Важное разграничение: стакан в памяти ядра и QUOTE_CACHE в Redis - это разные объекты с разным назначением. Стакан в ядре - рабочая структура матчинга, содержит все активные заявки и обновляется при каждой операции. QUOTE_CACHE в Redis - агрегированный снимок верхних 10x10 (или 5x5) уровней для публичного API; он заполняется MD-паблишером асинхронно и не участвует в критическом пути матчинга.

Важное разграничение по PostgreSQL (ORDER / TRADE): PostgreSQL заполняется асинхронно через Kafka-потребителя и не стоит на критическом пути матчинга. Ядро не ждёт подтверждения от PostgreSQL - оно пишет событие в журнал (синхронно) и в Kafka (затем). PostgreSQL получает батчи записей с задержкой в единицы-десятки мс. Это означает, что 110 000 тх/с в торговом ядре не транслируются в 110 000 синхронных write-транзакций PostgreSQL. Реальная нагрузка на PostgreSQL - это поток Kafka-потребителя, который может батчевать вставки.
...
- RISK_PARAM реплицируется в память каждого узла риск-чека - нулевая задержка на преторговый контроль.
...
| Kafka | идемпотентный producer (защита от дублей при ретрае) + дедупликация на стороне консьюмера по полю `seq`; группы потребителей по сервисам; retention 7 дней (активное потребление) - долгосрочный архив ведёт ClickHouse |
...
| PostgreSQL | WAL-archiving непрерывно + pg_basebackup раз в сутки, RAID 10 |
...
- Вход: поток заявок (новая limit/market, снятие, замена), упорядоченный sequencer'ом шарда (монотонный `seq` в рамках шарда).
- Ограничения: детерминизм (одинаковый вход -> одинаковый выход на primary и репликах), отсутствие потерь/дублей, ценовые коридоры.
- Результат: сделки + обновления стакана + подтверждения, каждое событие получает свой порядковый номер `seq` для воспроизводимости.

Пояснение по seq: это event-level монотонный счётчик, а не order-level. Каждое событие в журнале шарда - принятие заявки, сделка, изменение стакана - получает уникальный seq. Шарды имеют независимые счётчики, поэтому seq монотонен только в рамках одного шарда (то есть одного набора инструментов). Порядок между шардами не гарантирован - и не нужен, так как инструменты на разных шардах независимы.
...
Цены хранятся как целые числа (price / tick_size из справочника INSTRUMENT). Доступ к лучшей цене - O(1); вставка на новый уровень - O(log P), где P - число уровней (невелико).

### Ценовые коридоры (price bands)
Перед постановкой заявки в стакан ядро проверяет, что цена заявки не выходит за допустимый диапазон относительно последней сделки (или расчётной цены, если сделок ещё не было). Если цена выходит за коридор - заявка отклоняется с кодом "вне допустимого диапазона". Параметры коридора берутся из RISK_PARAM инструмента и загружаются в память ядра при старте.

### Последовательность работы (упрощённо)
```
on_new_order(o):
    seq_accept = sequencer.next()         # seq для события "заявка принята"
    log_event(seq_accept, ACCEPTED, o)

    if price_out_of_band(o.price):
        seq_rej = sequencer.next()
        log_event(seq_rej, REJECTED, o, reason=PRICE_BAND)
        return
    ...
    if o.side == BUY:
        while o.qty > 0 and ask.best_price <= limit_price:
            ...
            seq_trade = sequencer.next()              # seq для события "сделка"
            emit_trade(o, maker, trade_price, traded, seq_trade)
            log_event(seq_trade, TRADE, ...)
    ...
    seq_delta = sequencer.next()          # seq для события "изменение стакана"
    emit_book_delta(seq_delta)            # инкремент в MD-фид
```

Рыночная заявка может породить несколько сделок по разным ценам, проходя по уровням стакана - каждый `emit_trade` получает свой seq. Остаток рыночной заявки после исчерпания стакана отменяется (не ставится в стакан).
...
| In-memory, однопоточно на инструмент + sequencer per shard + event sourcing | seq монотонен только в рамках шарда; cross-shard ordering не гарантирован | выбран (для независимых инструментов cross-shard ordering не требуется; соответствует разделению матчинг/персистирование MOEX [[20]](https://www.moex.com/a7896)) |
...
Этот раздел описывает процесс управления гарантийным обеспечением на срочном рынке (SPECTRA). На фондовом рынке (ASTS) биржа не ведёт преторговый риск-чек: брокер заранее депонирует денежное обеспечение и самостоятельно контролирует остаток по клиентам.

Раздел описывает два уровня. Расчёт ГО (шаги 1-3, 5) выполняется клиринговым центром (НКЦ) по расписанию и находится вне MVP - он описан здесь для понимания, откуда берутся данные. Преторговый риск-чек (шаг 4) входит в MVP и выполняется на каждую заявку срочного рынка в реальном времени.

### Задача
Перед постановкой заявки на SPECTRA убедиться, что у участника достаточно обеспечения с учётом риска всего портфеля, и пересчитывать ГО/вариационную маржу по клиринговым сессиям. Метод соответствует портфельному маржированию НКЦ (аналог SPAN) [[25]](https://www.nationalclearingcentre.ru/catalog/030902)[[26]](https://www.moex.com/s1552)[[27]](https://www.moex.com/s1573).

### Входные данные и ограничения
- Позиции участника по инструментам SPECTRA, риск-параметры НКЦ: ставки рыночного риска 1/2/3 уровня, лимиты концентрации, ставки волатильности, спред-льготы [[25]](https://www.nationalclearingcentre.ru/catalog/030902).
- Параметры статические (минимальные уровни) + динамические (пересчёт вечером после торгов) [[26]](https://www.moex.com/s1552).
...
4. [MVP] Преторговый риск-чек: инкрементально пересчитать ГО с гипотетической новой позицией; если свободных средств хватает - заявка допускается в матчинг SPECTRA, иначе отклоняется. Все данные в памяти - обращений к БД нет.
5. [НКЦ, вне MVP] Вариационная маржа списывается/начисляется по клиринговым сессиям (дневной перерыв 14:00-14:03, вечерний 18:45-19:00) [[14]](https://cifra-broker.ru/help-center/grafik-raboty-moskovskoy-birzhi-vremya-torgov-i-sessiy/).
...
### Влияние на физическую схему и нагрузку
- RISK_PARAM и позиции SPECTRA хранятся в памяти узлов риск-чека (§6.4) -> преторговый чек укладывается в бюджет 300 мкс.
- Риск-чек выполняется только на заявки SPECTRA (~15 000 оп/с пик - пропорционально доле срочного рынка в общем потоке).
- MARGIN_STATE обновляется по результатам клиринговых сессий и по факту прохождения сделок через Kafka-потребителя - не на каждой входящей заявке.
...
| Kafka | Распределение событий ядра downstream | Высокая пропускная способность, надёжное хранение, группы потребителей; retention 7 дней (активное потребление); долгосрочный архив - ClickHouse |
| PostgreSQL | Заявки/сделки (recent), позиции, маржа, справочники | Транзакции, строгая согласованность, реплики; заполняется асинхронно через Kafka-потребителя, не стоит на критическом пути матчинга |
| ClickHouse | Журналы и архив тиков | Колоночное сжатие ~8-10x для аналитики/архива; основное долгосрочное хранилище событий |
...
| Sequencer / журнал событий | 3 реплики по кворуму на шард, синхронная запись (min.insync.replicas=2 - запись подтверждается только при наличии минимум 2 реплик) |
...
| Риск-чек | N+1, копия RISK_PARAM/позиций SPECTRA в памяти |
| PostgreSQL | 1 master + 2 реплики, WAL-archiving, RAID 10 |
...
Примечание по RAID: для PostgreSQL выбран RAID 10 (зеркалирование + страйп), а не RAID 6. RAID 6 имеет значительный write penalty из-за двойного parity расчёта, что критично для write-heavy workload WAL PostgreSQL. RAID 10 даёт лучшую write-производительность при приемлемой избыточности.

## 9.2 Целевые показатели RTO и RPO

| Компонент | RPO | RTO |
| :-------- | :-: | :-: |
| Торговое ядро (failover на standby) | 0 (синхронная репликация журнала) | ~10-30 секунд (promote standby + replay хвоста журнала) |
| Failover на резервный ЦОД | 0 (синхронный журнал) | минуты (переключение DNS/routing + инициализация) |
| PostgreSQL (master failover) | секунды (WAL lag) | ~1-2 минуты (promote реплики) |
| Redis (восстановление кэша) | не применимо (кэш) | ~30 секунд (replay из Kafka/RDB) |

Заявленная доступность 99,97% [[11]](https://www.dataspace.ru/company/press-center/it-dlya-investitsionnoy-ekosistemy/) соответствует не более ~2,6 часов суммарного downtime в год. При целевом RTO ядра ~10-30 секунд допустимо не более ~310-930 инцидентов в год - то есть на практике инцидентов с остановкой торгов должно быть единицы в год.

## 9.3 Поведение при отказах

| Отказ | Что происходит |
| :---- | :------------- |
| Падение primary-ядра | Promote standby (replay журнала), пауза торгов ~10-30 секунд |
...
flowchart TD
    ...
    subgraph COLO["ТОРГОВЫЙ КОНТУР"]
        ...
        subgraph ASTS_PATH["Фондовый / Валютный / Денежный рынок (ASTS)"]
            direction TB
            SEQ_A["Sequencer ASTS<br/>(per shard)"]:::core
            MEA["Матчинг ASTS<br/>[3 шарда]"]:::core
            JRN_A["Журнал событий<br/>[3 реплики / шард]"]:::core
            SEQ_A --> MEA --> JRN_A
        end

        subgraph SPEC_PATH["Срочный рынок (SPECTRA)"]
            direction TB
            RISK["Преторговый<br/>риск-чек"]:::core
            SEQ_S["Sequencer SPECTRA<br/>(per shard)"]:::core
            MES["Матчинг SPECTRA<br/>[2 шарда]"]:::core
            JRN_S["Журнал событий<br/>[3 реплики / шард]"]:::core
            RISK --> SEQ_S --> MES --> JRN_S
        end

        MDP["MD-паблишеры<br/>[N+1]"]

        PROF -->|SBE, kernel-bypass| GW
        GW -->|ASTS инструменты| SEQ_A
        GW -->|SPECTRA инструменты| RISK
        JRN_A -->|события| MDP
        JRN_S -->|события| MDP
    ...
    JRN_A -->|поток событий| KAFKA[(Kafka<br/>rf=3, retention 7d)]:::mq
    JRN_S -->|поток событий| KAFKA
    ...
    subgraph DR["РЕЗЕРВНЫЙ ЦОД M1"]
        direction TB
        JRN2["Реплика журналов ядра"]:::core
        PGR["Реплики БД"]:::db
    end

    JRN_A -->|синхронная репликация| JRN2
    JRN_S -->|синхронная репликация| JRN2
    PGO -.-> PGR
    PGP -.-> PGR
```
...
Потоки данных:
1. Участник торгов шлёт заявку (SBE) -> шлюз -> маршрутизация по типу рынка. Для ASTS: sequencer присваивает seq -> матчинг. Для SPECTRA: преторговый риск-чек -> sequencer -> матчинг.
...
3. Журнал асинхронно вытекает в Kafka -> PostgreSQL (заявки/сделки/позиции/маржа), ClickHouse (журналы + архив), Redis (котировочный кэш). PostgreSQL не стоит на критическом пути: матчинг не ждёт его подтверждения.
...
| Риск-чек (только SPECTRA) | до 15k оп/с пик | 16 | 128 ГБ | 500 ГБ | 25 Гбит |
| MD-паблишеры | до 220k сообщ/с пик | 16 | 64 ГБ | 500 ГБ | 25 Гбит |
| ClickHouse | архив ~12,5 ТБ/5 лет | 32 | 256 ГБ | 20 ТБ | 25 Гбит |
| PostgreSQL (заявки/сделки) | Kafka-потребитель, батчевые вставки | 32 | 256 ГБ | 8 ТБ NVMe | 25 Гбит |
| PostgreSQL (позиции/маржа) | Kafka-потребитель, батчевые вставки | 32 | 128 ГБ | 2 ТБ | 10 Гбит |
...
Примечание по PostgreSQL (заявки/сделки): сервис заполняется асинхронно через Kafka-потребителя и не стоит на критическом пути матчинга. Пиковая нагрузка определяется скоростью потребления из Kafka, а не скоростью матчинга. Батчевые вставки значительно снижают давление на WAL по сравнению с построчными.

Примечание по риск-чеку: пиковая нагрузка ~15 000 оп/с - это оценка для SPECTRA, пропорционально его доле в суммарном потоке 110 000 тх/с. Все данные (RISK_PARAM, позиции) в памяти, обращений к БД нет.
...
| Sequencer/журнал | Bare | 16 / 128 ГБ / 4 ТБ NVMe | 3 | кворум (per shard, объединено) |
| Шлюзы FIX/TWIME/SIMBA | Bare | 16 / 64 ГБ / 0,5 ТБ | 6 | N+1 (§4.1) |
| MD-паблишеры | Bare | 16 / 64 ГБ / 0,5 ТБ | 6 | N+1 + A/B |
| Риск-чек (SPECTRA) | Bare | 16 / 128 ГБ / 0,5 ТБ | 4 | N+1 |
| PostgreSQL заявки/сделки | Bare | 32 / 256 ГБ / 8 ТБ NVMe | 9 | 3 шарда x (master+2 реплики) |
...
| Итого | | | ~86 | + зеркало ядра и журналов в резервном ЦОД |
