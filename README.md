...
Этот раздел описывает два процесса. Расчёт ГО (шаги 1-3, 5) выполняется клиринговым центром (НКЦ) по расписанию и находится вне MVP - он описан здесь для понимания, откуда берутся данные. Преторговый риск-чек (шаг 4) входит в MVP и выполняется на каждую заявку в реальном времени. Этот алгоритм актуален только для срочного рынка.
